...
def test_load_classification_line(line, classification):
    ...
    loaded_line = export_to_responses.load_classification_line(line)
    assert loaded_line == classification


def test_explode_classifications(classification):

    flat_classifications = export_to_responses.explode_classification(
        classification)
    result = pd.DataFrame(flat_classifications)  # list of series to DataFrame
    assert set(result.columns.values) == set(['task','task_label','value','user_id','classification_id','created_at','workflow_version','subject_ids'])
...
def test_sanitise_string():
    ...
    cleaned_strings = list(map(export_to_responses.sanitise_string, markdown_strings))
...
def test_rename_response(response_without_markdown, schema, renamed_response):
    ...
    result = export_to_responses.rename_response(response_without_markdown, schema)
    assert all(result == renamed_response)



def test_clean_response(response, schema, cleaned_response):
    cleaned = export_to_responses.clean_response(response, schema)
